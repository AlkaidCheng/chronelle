...
} from "../src/expenses/draft-store";
import { emptyExpenseFields } from "../src/expenses/editor";
...
const expenseId = "019d6e7d-0000-7000-8000-000000000004";

function draft(
  change: Partial<ExpenseDraftSnapshot> = {},
): ExpenseDraftSnapshot {
  const baseline = emptyExpenseFields(
    "UTC",
    null,
...
describe("Mini Program Expense draft store", () => {
  it("isolates creation and editing drafts by user, workspace and Event", async () => {
    const store = new ExpenseDraftStore(
      new MemoryStorage(),
      () => new Date("2030-01-02T00:00:00Z"),
    );
    const create = draft();
    const edit = draft({ commandId: null, expenseId, sourceVersion: 3 });
    ...
    await expect(
      store.load({ ...create, userId: expenseId }),
    ).resolves.toBeNull();
    await expect(
      store.load({ ...create, eventId: expenseId }),
    ).resolves.toBeNull();
    await store.remove(create);
    await expect(store.load(edit)).resolves.toEqual(edit);
  });

  it("rejects malformed stored fields and does not adopt a broken draft", async () => {
    const storage = new MemoryStorage();
    storage.values.set(expenseDraftStorageKey, {
      drafts: [
        draft({ fields: { ...draft().fields, sectionId: "not-an-id" } }),
      ],
      version: 1,
    });
    const store = new ExpenseDraftStore(
