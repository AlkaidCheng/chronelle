...
function deviceTimeZone(fallback: string | null): string {
  try {
    return (
...
function EventEditor({
  ...
  const latestDraft = useRef<EventDraftSnapshot | null>(null);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      void (
        dirty
          ? runtime.eventDrafts.save(draft)
          : runtime.eventDrafts.remove(identity)
  ...
  useEffect(
    () => () => {
      ...
      if (
        current !== null &&
        !sameEventFields(current.fields, current.baseline)
      )
        void runtime.eventDrafts.save(current).catch(() => undefined);
    },
    [runtime],
  );
