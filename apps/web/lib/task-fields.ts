...
export function readTaskFields(
  task?: Pick<TaskResponse, "displayName" | "dueOn" | "dueAt">,
) {
  const [dueDate = "", dueTime = ""] =
    task?.dueOn !== null && task?.dueOn !== undefined
      ? [task.dueOn, ""]
      : toDateTimeInput(task?.dueAt ?? null).split("T");
  return { displayName: task?.displayName ?? "", dueDate, dueTime };
}
...
export function taskFieldsPayload(
  ...
  if (fields.dueDate === "") {
    ...
    return { displayName: fields.displayName, dueOn: null, dueAt: null };
  }
