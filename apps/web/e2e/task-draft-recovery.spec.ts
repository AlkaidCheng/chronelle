...
import {
  exerciseTaskRecovery,
  revisitTaskView,
} from "./helpers/task-draft-recovery";
...
test("recovers Task creation and edits through browser navigation", async ({
  page,
}, testInfo) => {
  await signIn(page);
...
for (const outcome of ["success", "lost response"] as const)
  test(`settles a Task creation ${outcome} after leaving its event`, async ({
    page,
    request,
  }) => {
    ...
    await expect(page).toHaveURL(todosUrl);
    await expect(
      page.getByRole("row").filter({ hasText: "Confirm the lantern delivery" }),
    ).toHaveCount(1);
    ...
    const canonical = await request.get(`/api/tasks/${resourceId}`, {
      headers,
    });
    ...
    const todos = await request.get(`/api/events/${eventId}/todos`, {
      headers,
    });
