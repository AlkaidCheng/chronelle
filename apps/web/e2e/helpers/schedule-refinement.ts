...
export async function exerciseScheduleRefinement(
  ...
  await openDatePanel(editor, datesRow(editor));
  ...
  // Closed, the row reads the span above the times row.
  await closeDatePanel(editor);
  const viewport = page.viewportSize();
  await page.setViewportSize({ width: 320, height: 568 });
  for (const colorScheme of ["light", "dark"] as const) {
    await page.emulateMedia({ colorScheme });
    await expect(datesRow(editor)).toBeInViewport();
    await expect(timesRow(editor)).toBeInViewport();
    expect(
  ...
  // Typing the start alone leaves a one-day span; a press on the start day
  // keeps it.
  await openDatePanel(editor, datesRow(editor));
  const typed = editor.getByLabel("Type a date", { exact: true });
  await typed.fill("Feb 28, 2028");
  await expectDates(editor, "Feb 28, 2028");
  await editor.getByRole("button", { name: dayName("2028-02-28") }).click();
  await expectDates(editor, "Feb 28, 2028");
  await closeDatePanel(editor);
  // The times row opens the panel with the times unfolded.
  await openDatePanel(editor, timesRow(editor));
  const startTime = editor.getByLabel("Start", { exact: true });
  const endTime = editor.getByLabel("End", { exact: true });
  ...
  await closeDatePanel(editor);
  await expect(timesRow(editor)).toHaveText("Times: 10:00 AM to 11:00 AM");
  await save.click();
  await expect(
