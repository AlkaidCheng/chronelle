...
describe.each(Object.entries(backends))(
  ...
  (_backend, compose) => {
    it("creates the three accounts and their data once, and leaves an existing set alone", async () => {
      ...
      const friends = (
        ...
        sent: { kind: string; channel: string | null; email: string | null }[];
      };
      ...
      ).json() as { items: { displayName: string; location: string | null }[] };
      ...
      expect(shares.pending.map((share) => share.person?.displayName)).toEqual([
        "The Tanakas",
      ]);
      const trash = (
        await app.inject({
    ...
    it("refuses to continue when an account exists with another password", async () => {
      app = buildApp(compose(database, email));
