...
`0028_add_backend_readiness_function.sql` add the Event, Task, Expense, and
...
reference read, the document transfer records, the identity sign-in, and
the backend readiness check used by the CloudBase rpc path; 0013 also
introduces the shared object write core the family functions delegate to. They change no tables and need
