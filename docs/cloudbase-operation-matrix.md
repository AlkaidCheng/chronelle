...
| Sharing and permission-scope changes | Grant, revoke, stop inheritance                        | Canonical scope, inherited access, audit, workspace isolation                                          | `ShareWriteRepository` and `PermissionScopeWriteRepository`; CloudBase adapter calls `chronelle_resource_share`, `chronelle_resource_share_revoke`, and `chronelle_object_scope_update` (migration 0020) behind `CLOUDBASE_WRITES_ENABLED`                                                              |
