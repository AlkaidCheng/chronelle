...
| Reminder create                      | `POST /api/reminders`                                  | Authorization, typed validation, audit event, generated identity            | PostgreSQL; next family to port on the shared object write core                                                                   |
| Reminder update                      | `PATCH /api/reminders/:id`                             | Authorization, CAS on `version`, audit event, no partial typed row          | PostgreSQL; next family to port on the shared object write core                                                                   |
