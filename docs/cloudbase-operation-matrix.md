...
| Expense, reminder create             | Typed creation routes                                  | Authorization, typed validation, audit event, generated identity            | PostgreSQL; next families to port on the shared object write core                                                                 |
| Expense, reminder update             | Typed-resource `PATCH` routes                          | Authorization, CAS on `version`, audit event, no partial typed row          | PostgreSQL; next families to port on the shared object write core                                                                 |
