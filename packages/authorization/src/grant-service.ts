...
export class ResourceGrantService {
  ...
  async share(
    ...
    await this.#assertCanShare(context.principal, input.resourceId);
    const [principal, duplicatePrincipal] = await this.#database
      .select({
        id: users.id,
  ...
  async revoke(
    ...
    await this.#assertCanShare(context.principal, grant.resourceId);
